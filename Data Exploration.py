...
def get_accuracies(model, features_train, class_train, features_test, class_test):
    class_train_predication = model.predict(features_train)
    train_accuracy = accuracy_score(class_train, class_train_predication)
    
    class_test_predication = model.predict(features_test)
    test_accuracy = accuracy_score(class_test, class_test_predication)

    return train_accuracy, test_accuracy


train_accuracy, test_accuracy = get_accuracies(model, features_train, fetal_health_train, features_test, fetal_health_test)
